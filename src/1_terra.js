class Terra {
    ...
    constructor({
        ...
        this.creaturesConfigs = this.creaturesConfigs;
    ...
    load(map) {
        ...
        if (!map.length === this.size ** 2) {
            throw new Error('map dimensions are distorted');
        }
        this.creatures = [];
        this.steps = 0;
        ...
        for( let x=0; x<this.size; x++){
            for( let y=0; y<this.size; y++){
                ...
                const Constructor = getCreatureBySymbol(symbol);
        ...
        y = +y;
         ...
         if(!isNaN(x) && !isNaN(y)) {
             ...
             if(index < symbols.length) {
                 const C = getCreatureBySymbol(symbols[index]);
    ...
    editMode(isEdited) { 
        ...
        if(isEdited) {
            ...
            container.removeEventListener('click', this.editListener);
        }
